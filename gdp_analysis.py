...
GROWTH_CRITERIA = 0.2  # 20 percent
...
def analyze_gdp_growth(data, country_names, target_year):
    ...
    for country_code, growth_data in data.items():
        ...
        try:
            ...
            prev_growth_rates = [
                growth_data[year] for year in prev_years if year in growth_data
            ]
            ...
            if target_growth > avg_prev_growth * (1 + GROWTH_CRITERIA):
...
def main():
    ...
    try:
        ...
        results, investigated_countries = analyze_gdp_growth(
            gdp_data, country_names, target_year)

        print("\n===== GDP Growth Analysis =====\n")
        print(f"Target Year: {target_year}\n")
        print(f"Number of countries investigated: {investigated_countries}")

        criteria_description = f"Countries where the target year's growth rate is higher than the {GROWTH_CRITERIA*100}% of the previous three years"
        ...
        if results:
            headers = [
                "Country", "Three-Year Average Growth (%)",
                "Target Year Growth (%)"
            ]
            table = [(country, f"{avg_growth:.2f}", f"{target_growth:.2f}")
                     for country, avg_growth, target_growth in results]
