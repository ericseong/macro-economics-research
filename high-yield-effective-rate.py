...
def get_spread_data(fred_api_key: str, years: int):
    ...
    series = fred.get_series("BAMLH0A0HYM2EY", observation_start=start_date)
...
def plot_combined(spread_series, spy_series_raw):
    ...
    # Layout
    fig.update_layout(
        title="High-Yield Effective Yield vs. SPY (Overlapped, with MA10)",
        dragmode="pan",
        xaxis=dict(
            title="Date",
        ...
        yaxis=dict(
            title=dict(text="Yield (%)", font=dict(color="firebrick")),
            side="left",
            showgrid=True,
            showline=True,
