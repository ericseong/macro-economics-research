...
def load_or_synthesize_daily_data(n_days: int = 3 * 365, seed: int = 42) -> pd.DataFrame:
    ...
    if agsi_key:
        try:
            ...
            df["storage_fill_pct"] = df.index.to_series().map(agsi["storage_fill_pct"]).combine_first(df["storage_fill_pct"])
    ...
    try:
        ...
        df["wind_index"] = df.index.to_series().map(wind["wind_index"]).combine_first(df["wind_index"])
        print("--- Open Meteo wind index ---")
        print(df)
    except Exception as e:
    ...
    if ttf_csv and os.path.exists(ttf_csv):
        ...
        try:
            ...
            df["ttf_eur_mwh"] = df.index.to_series().map(ts["value"]).combine_first(df["ttf_eur_mwh"])
        except Exception as e:
    ...
    if eua_csv and os.path.exists(eua_csv):
        ...
        try:
            ...
            df["eua_eur_t"] = df.index.to_series().map(ts["value"]).combine_first(df["eua_eur_t"])
        except Exception as e:
        ...
        try:
            ...
            if auction_path:
                ts = load_csv_series(auction_path, date_col="Date", value_col="Auction Price €/tCO2").set_index("date").sort_index()
                # Join to daily index and forward-fill across non-auction days
                df = df.join(ts["value"].rename("eua_from_auction"), how="left")
                df["eua_eur_t"] = df["eua_from_auction"].ffill().combine_first(df["eua_eur_t"])
                df.drop(columns=["eua_from_auction"], inplace=True)
                print("[INFO] Loaded EUA auction CSV:", auction_path)
                print("--- EU carbon permit price ---")
                print(df)
            ...
            print("[WARN] EUA auction CSV/NDL load failed:", e)
    ...
    if jkm_csv and os.path.exists(jkm_csv):
        ...
        try:
            ...
            df["jkm_usd_mmbtu"] = df.index.to_series().map(ts["value"]).combine_first(df["jkm_usd_mmbtu"])
        except Exception as e:
...
def build_arg_parser() -> argparse.ArgumentParser:
    ...
    p.add_argument(
        ...
        type=float,
        default=0.01,
        help="Directional threshold (e.g., 0.01 = 1%%)"
    )
    ...
    p.add_argument("--mode", choices=["regress", "classify"], default="regress",
                   help="Weekly forecast mode: regress (pct) or classify (direction)")
    return p
