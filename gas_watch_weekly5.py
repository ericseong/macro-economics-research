...
----------------------------
- Validation **R²**, **Adjusted R²**, and **Price-level R²** (how much next-week price
  variation is explained by the model-implied next-week price).
- Automatic **R² interpretation** with concrete suggestions to improve the model.
- Plotly chart showing **model-implied next-week price vs. realized** next-week price
  across the validation span.
...
Regression (pct change with guardrail + TS-CV):
    python gas_watch_weekly.py --run-all \
      ...
      --plotly-regression-html-out ./output/gas_regression_results.html
...
def _apply_overlay(base: pd.Series, live: pd.Series, prov: pd.DataFrame, col: str, label: str) -> pd.Series:
    ...
    If live has NaNs, they are filled by forward filling the last known live value.
    """
    # Align live to base's index (including forward-filling missing values)
    mapped = base.index.to_series().map(live).ffill()

    # Create a mask where live provided a non-null (possibly carried forward) value
    mask = mapped.notna()

    # Copy base so we don't mutate it
    out = base.copy()

    # Replace base values with interpolated/forward-filled live values
    out[mask] = mapped[mask].values

    # Record provenance where replacement occurred
...
def fetch_yahoo_ttf(start_date: str, end_date: str) -> pd.DataFrame:
    ...
    for tkr in tickers:
        try:
            ...
            print("--- [DEBUG] TTF from yfinance")
            print(out[["date", "value"]].head(20))
...
def build_weekly_dataset(daily: pd.DataFrame) -> Tuple[pd.DataFrame, pd.Series, pd.Series]:
    """Return (X, y, weekly_ttf) with one-step lookahead alignment.
    ...
    X = X.loc[valid]
    y = y.loc[valid]
    weekly_ttf = ttf.loc[valid]

    # Safety: drop any remaining rows with NaNs
    mask_no_nan = X.notna().all(axis=1)
    X = X.loc[mask_no_nan]
    y = y.loc[mask_no_nan]
    weekly_ttf = weekly_ttf.loc[mask_no_nan]

    return X, y, weekly_ttf
...
def train_ridge(
    X: pd.DataFrame,
    y: pd.Series,
    weekly_ttf: Optional[pd.Series] = None,
    alpha: float | None = None
) -> Tuple[Pipeline, Dict[str, float], pd.Series, pd.Index]:
    """Train StandardScaler + Ridge with optional TS-CV alpha tuning.
    Returns:
        pipe: trained pipeline
        metrics: dict containing r2, adjusted r2, price-level r2, MAE, RMSE, etc.
        y_hat_val: validation predictions (returns) as a pd.Series
        val_index: index of the validation slice (pd.Index)
    """
    n = len(X)
    if n < 30:
    ...
    # TS-CV to pick alpha on return-level R²
    for a in grid:
        scores = []
    ...
    # Holdout validation split (80/20, time-ordered)
    split = int(n * 0.8)
    X_train, X_val = X.iloc[:split], X.iloc[split:]
    y_train, y_val = y.iloc[:split], y.iloc[split:]
    pipe.fit(X_train, y_train)

    y_hat_val = pd.Series(pipe.predict(X_val), index=X_val.index, name="y_pred_val")

    # Return-level metrics on validation
    r2_val = float(r2_score(y_val, y_hat_val))
    mae_val = float(mean_absolute_error(y_val, y_hat_val))
    rmse_val = float(np.sqrt(mean_squared_error(y_val, y_hat_val)))

    # Adjusted R² on validation
    p = X.shape[1]
    n_val = len(y_val)
    adj_r2 = float(1 - (1 - r2_val) * (n_val - 1) / max(1, (n_val - p - 1))) if n_val > p + 1 else float("nan")

    # Price-level R²: how well we explain next-week price (not just returns)
    price_r2 = float("nan")
    if weekly_ttf is not None:
        p_t = weekly_ttf.loc[X_val.index]                     # P_t
        p_t1_true = weekly_ttf.shift(-1).loc[X_val.index]     # P_{t+1} true
        p_t1_pred = (p_t * (1.0 + y_hat_val)).rename("P_next_pred")
        idx = p_t1_true.dropna().index.intersection(p_t1_pred.dropna().index)
        if len(idx) >= 5:
            price_r2 = float(r2_score(p_t1_true.loc[idx], p_t1_pred.loc[idx]))

    # Dispersion of TS-CV scores (on full series)
    scores = []
    for tr, va in tscv.split(X):
        pipe.fit(X.iloc[tr], y.iloc[tr])
        yhat = pipe.predict(X.iloc[va])
        scores.append(r2_score(y.iloc[va], yhat))
    tscv_std = float(np.std(scores)) if scores else float("nan")
    ...
    return pipe, metrics, y_hat_val, X_val.index
...
def make_weekly_factor_contrib_plot(
    pipe: Pipeline,
    X: pd.DataFrame,
    weekly_ttf: pd.Series,
    span_index: Optional[pd.Index] = None,
    top_n: int = 6,
    title: str = "Weekly Factor Contributions → Next-Week Price",
    ...
    Xw = X.loc[span_index]
    if Xw.empty:
        print("[WARN] Span index produced empty slice.")
        return None

    # standardize and get per-feature contributions to predicted return
    Z = (Xw.values - scaler.mean_) / scaler.scale_
    contrib = pd.DataFrame(Z * beta, index=Xw.index, columns=Xw.columns)

    y_pred = contrib.sum(axis=1).rename("y_pred")
    P_t   = weekly_ttf.loc[Xw.index]
    P_t1_pred = (P_t * (1.0 + y_pred)).rename("P_next_pred")
    P_t1_true = weekly_ttf.shift(-1).loc[Xw.index].rename("P_next_true")

    # keep top-N contributors; bucket the rest as "Other"
    avg_abs = contrib.abs().mean(axis=0).sort_values(ascending=False)
    keep = avg_abs.index[:top_n].tolist()
    if len(avg_abs) > top_n:
        contrib["Other"] = contrib.loc[:, ~contrib.columns.isin(keep)].sum(axis=1)
        plot_cols = keep + ["Other"]
    else:
        plot_cols = keep

    fig = make_subplots(
        rows=2, cols=1, shared_xaxes=True, vertical_spacing=0.08,
        subplot_titles=("Next-Week Price (Realized vs Model)", "Weekly Factor Contributions to Predicted Return")
    )

    # top panel: prices
    df_price = pd.concat([P_t1_true, P_t1_pred], axis=1).dropna()
    if not df_price.empty:
        fig.add_trace(go.Scatter(x=df_price.index, y=df_price["P_next_true"], mode="lines", name="Realized next-week price"), row=1, col=1)
        fig.add_trace(go.Scatter(x=df_price.index, y=df_price["P_next_pred"], mode="lines", name="Model-implied next-week price"), row=1, col=1)
    else:
        print("[WARN] Not enough overlap to draw price lines; plotting contributions only.")

    # bottom panel: stacked contributions
    for col in plot_cols:
        s = contrib[col]
        fig.add_trace(go.Bar(x=s.index, y=s.values, name=col), row=2, col=1)

    fig.update_layout(
        title=title, barmode="relative", height=700,
        legend=dict(orientation="h", yanchor="bottom", y=1.02, xanchor="left", x=0),
        margin=dict(t=70, b=40, l=60, r=20),
    )
    fig.update_yaxes(title_text="€/MWh", row=1, col=1)
    fig.update_yaxes(title_text="Contribution to return (unitless)", row=2, col=1)
    ...
    if out_html:
        ...
        print(f"[INFO] Weekly factor contributions chart saved to: {out_html}")
...
def make_regression_vs_price_plot(
    weekly_ttf: pd.Series,
    y_pred_val: pd.Series,
    val_index: pd.Index,
    title: str = "Regression: Model-Implied vs Realized Next-Week TTF",
    out_html: Optional[str] = None,
    show: bool = False,
):
    """Plot model-implied next-week price against realized next-week price over validation."""
    if go is None or make_subplots is None:
        print("[WARN] Plotly not available. Install with: pip install plotly")
        return None

    # Align validation span
    p_t = weekly_ttf.loc[val_index]                     # P_t
    p_t1_true = weekly_ttf.shift(-1).loc[val_index]     # P_{t+1} realized
    p_t1_pred = (p_t * (1.0 + y_pred_val)).rename("P_next_pred")

    df = pd.DataFrame({
        "P_t": p_t,
        "P_next_true": p_t1_true,
        "P_next_pred": p_t1_pred,
    }).dropna()

    if df.empty:
        print("[WARN] Not enough overlapping data to plot regression vs price.")
        return None

    fig = go.Figure()
    fig.add_trace(go.Scatter(x=df.index, y=df["P_next_true"], mode="lines", name="Realized next-week price"))
    fig.add_trace(go.Scatter(x=df.index, y=df["P_next_pred"], mode="lines", name="Model-implied next-week price"))

    fig.update_layout(
        title=title,
        xaxis_title="Week (ending Fri)",
        yaxis_title="€/MWh",
        height=500,
        legend=dict(orientation="h", yanchor="bottom", y=1.02, xanchor="left", x=0),
    )
    ...
    if out_html:
        pio.write_html(fig, out_html, include_plotlyjs="cdn", full_html=True)
        print(f"[INFO] Regression results plot saved to: {out_html}")
...
def _interpret_r2_and_suggest(r2: float) -> Tuple[str, List[str]]:
    """Return a short interpretation message and concrete improvement ideas based on R²."""
    if not np.isfinite(r2):
        return ("R² is not available for the validation set.", [])
    if r2 < 0:
        msg = ("R² < 0 on validation: the linear model underperforms a constant mean; "
               "signals likely weak or mis-specified for one-week returns.")
    elif r2 < 0.05:
        msg = ("Very low R² on validation: weekly returns are dominated by noise; "
               "the current features explain only a tiny share of variation.")
    elif r2 < 0.20:
        msg = ("Low but non-zero R²: there is some explanatory power, though the majority "
               "of week-ahead variation remains unexplained.")
    elif r2 < 0.40:
        msg = ("Moderate R²: a meaningful fraction of week-ahead variation is captured; "
               "useful directional signal with residual noise.")
    else:
        msg = ("High R² for financial weekly returns: model fits unusually well; "
               "watch for regime shifts and overfitting.")

    suggestions = [
        "Add **lags** and short-term **momentum** terms (e.g., ΔTTF, 4-week momentum, volatility).",
        "Capture **seasonality** and calendar effects (month, quarter, holiday weeks, maintenance seasons).",
        "Introduce **non-linearities** (PolynomialFeatures, tree-based models, splines) and **interactions** (e.g., wind × HDD).",
        "Engineer **spread** and **relative value** features (TTF–JKM, TTF–Brent gas-oil parity, EUA × HDD).",
        "Try **longer horizons** or **nowcasts** (blend daily signals into a nowcast for next week’s average).",
        "Segment by **regime** (storage high/low, crisis periods) or use time-varying parameters.",
        "Expand **data coverage** (LNG flows, pipeline outages, imports/exports, merit-order proxies).",
        "Use **regularization tuning** and **walk-forward validation**; compare Ridge/Lasso/ElasticNet and tree ensembles.",
    ]
...
def make_report(
    ...
    if metrics.get("mode") == "classify":
        ...
        interpret_title, interp_msg, improve_title, improve_lines = "", "", "", ""
    else:
        r2   = metrics.get("r2",   float("nan"))
        adj  = metrics.get("adj_r2", float("nan"))
        pr2  = metrics.get("price_r2", float("nan"))
        ...
        model_line = (
            "Ridge(alpha={alpha:.2f})  |  R² (val): {r2:.3f}  |  Adj R²: {adj:.3f}  |  "
            "Price-level R²: {pr2:.3f}  |  MAE: {mae:.4f}  |  RMSE: {rmse:.4f}  | "
            "Train/Val weeks: {ntr}/{nva}"
        ).format(alpha=alpha_used, r2=r2, adj=adj, pr2=pr2, mae=mae, rmse=rmse, ntr=ntr, nva=nva)
        if tmean is not None:
            model_line += "  |  TS-CV R²: {m:.3f} ± {s:.3f}".format(m=tmean, s=tstd)

        # Interpretation & improvement suggestions
        interp_msg, suggestions = _interpret_r2_and_suggest(r2)
        interpret_title = "## R² Interpretation"
        improve_title = "## How to Improve the Model"
    ...
    # Only add interpretation when in regression mode
    if metrics.get("mode") != "classify":
        parts += [
            "",
            interpret_title,
...
def run_pipeline(args: argparse.Namespace) -> None:
    ...
    # Build weekly data (X features, y returns, plus weekly prices for price-level R² & plotting)
    X, y, weekly_ttf = build_weekly_dataset(daily)
    ...
    if args.mode == "classify":
        ...
        pipe, metrics, y_pred_val, val_idx = train_ridge(X, y, weekly_ttf=weekly_ttf, alpha=None)
        ...
        print(report_md)
        payload = {
            "asof_week": asof_week.strftime("%Y-%m-%d"),
            "predicted_return": pred_ret,
            "direction": direction,
            "metrics": metrics,
            "top_contributions": contribs[: args.topk]
        }

        # Regression results plot vs price (validation period)
        if args.plotly_regression_html_out or args.plotly_regression_show:
            title = f"Model-Implied vs Realized Next-Week TTF (val set; as of {pd.Timestamp.today().date()})"
            make_regression_vs_price_plot(
                weekly_ttf=weekly_ttf,
                y_pred_val=y_pred_val,
                val_index=val_idx,
                title=title,
                out_html=(args.plotly_regression_html_out or None),
                show=args.plotly_regression_show,
            )
...
def build_arg_parser() -> argparse.ArgumentParser:
    ...
    # NEW: regression result plot (pred vs real price)
    p.add_argument("--plotly-regression-html-out", type=str, default="", help="Save a Plotly chart of model-implied vs realized next-week TTF (validation span)")
    p.add_argument("--plotly-regression-show", action="store_true", help="Show the regression results chart in a window if supported")
    return p
