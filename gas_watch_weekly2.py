...
def load_eua_auction_series(path: str) -> pd.Series:
    ...
    def _pick_for_day(g):
        eu = g[g["country"] == "EU"].dropna(subset=["price"])
        if not eu.empty:
...
# ---- Yahoo helpers (fallbacks for TTF/Brent) ----
...
def fetch_yahoo_eur_per_usd(start_date: str, end_date: str) -> pd.DataFrame:
    """
    Fetch EUR per 1 USD using Yahoo FX.
    Yahoo's EURUSD=X (aka EUR=X) is USD per 1 EUR; we invert it.
    Returns: DataFrame[date, eur_per_usd]
    """
    try:
        import yfinance as yf
    except ImportError as e:
    ...
    # Override with: export YF_EURUSD_TICKERS="EURUSD=X,EUR=X"
    tickers_env = os.getenv("YF_EURUSD_TICKERS", "").strip()
    tickers = [t.strip() for t in tickers_env.split(",") if t.strip()] or ["EURUSD=X", "EUR=X"]
    ...
    for tkr in tickers:
        try:
            df = yf.download(tkr, start=start_date, end=end_plus, interval="1d",
                             auto_adjust=False, progress=False, threads=False)
            s = _to_1d_price(df)  # USD per EUR
            if s is None or s.empty:
                continue
            s.index = pd.to_datetime(s.index).tz_localize(None)
            s = (1.0 / s).rename("eur_per_usd")  # invert to EUR per USD
            out = s.sort_index().reset_index().rename(columns={"index": "date", "Date": "date"})
            out["date"] = pd.to_datetime(out["date"])
            return out[["date", "eur_per_usd"]]
        except Exception:
            continue

    raise RuntimeError("Yahoo FX: no EURUSD ticker returned data. Try setting YF_EURUSD_TICKERS.")
...
def load_or_synthesize_daily_data(n_days: int = 3 * 365, seed: int = 42, return_provenance: bool = False):
    ...
    # NEW: provenance table initialized as synthetic
    ...
    # 4) EUR per USD (ECB primary, Yahoo fallback)
    fx_loaded = False
    try:
        fx = fetch_ecb_eur_usd(start_date, end_date).set_index("date").sort_index()
        if not fx.empty and "eur_per_usd" in fx.columns:
            ...
            if filled.notna().any():
                ...
                print("[WARN] ECB FX mapped all NaN; will try Yahoo fallback")
        else:
            print("[WARN] ECB FX returned empty; will try Yahoo fallback")
    except Exception as e:
        print("[WARN] ECB FX fetch failed:", e)

    if not fx_loaded:
        try:
            fx2 = fetch_yahoo_eur_per_usd(start_date, end_date).set_index("date").sort_index()
            mapped2 = df.index.to_series().map(fx2["eur_per_usd"])
            present_mask2 = mapped2.notna()
            filled2 = mapped2.ffill().bfill()
            if filled2.notna().any():
                df["eur_per_usd"] = filled2
                prov.loc[present_mask2, "eur_per_usd"] = "yahoo"
                prov.loc[(~present_mask2) & filled2.notna(), "eur_per_usd"] = "yahoo-ffill"
                fx_loaded = True
                print("[INFO] Used Yahoo FX for eur_per_usd")
            else:
                print("[WARN] Yahoo FX mapped all NaN; leaving eur_per_usd synthetic")
        except Exception as e:
            print("[WARN] Yahoo FX fallback failed:", e)
    ...
    if eua_csv and os.path.exists(eua_csv):
        ...
        try:
            ...
            if auction_path:
                s = load_eua_auction_series(auction_path)
                # Fill the few missing-price days by interpolation; keep edges stable
                s = s.astype(float).sort_index().interpolate(limit_direction="both")
                eua_series = s
                print(f"[INFO] EUA from auction CSV: {auction_path}")
        except Exception as e:
            print("[WARN] EUA auction CSV parse failed:", e)

    if eua_series is None:
        ndl_key = get_ndl_api_key()
        if ndl_key and os.getenv("NDL_EUA_CODE", "").strip():
            try:
                ts = fetch_ndl_timeseries(os.getenv("NDL_EUA_CODE").strip(), ndl_key, start_date, end_date)
                eua_series = ts.set_index("date")["value"].sort_index()
                print("[INFO] EUA from NDL:", os.getenv("NDL_EUA_CODE").strip())
            except Exception as e:
                print("[WARN] NDL EUA fetch failed:", e)

    if eua_series is not None:
        # Map daily index to auction series (exact auction days)
        mapped = df.index.to_series().map(eua_series)

        # Forward-fill auction prices across non-auction days
        filled = mapped.ffill()

        # Masks to mark provenance correctly
        mask_present = mapped.notna()                       # auction dates with an actual price
        mask_ffill   = filled.notna() & (~mask_present)     # non-auction dates filled from last auction

        # Overwrite synthetic baseline with real values
        df.loc[mask_present, "eua_eur_t"] = mapped.loc[mask_present].values
        df.loc[mask_ffill,   "eua_eur_t"] = filled.loc[mask_ffill].values

        # Tag provenance: 'auction' for real auction days, 'auction-ffill' for carry-forward days
        prov.loc[mask_present, "eua_eur_t"] = "auction"
        prov.loc[mask_ffill,   "eua_eur_t"] = "auction-ffill"

    # JKM (read local CSV directly; provenance: csv vs csv-ffill)
    try:
        base = Path(__file__).resolve().parent if "__file__" in globals() else Path(".").resolve()
        candidates = [
            base / "LNGJKM.csv",
            base / "data" / "LNGJKM.csv",
            base / "../data" / "LNGJKM.csv",
            Path("LNGJKM.csv"),
        ]
        jkm_path = next((p for p in candidates if p.exists()), None)
    except Exception:
        jkm_path = None

    if jkm_path is not None:
        try:
            # Investing.com export typically has columns: Date, Price
            ts = load_csv_series(str(jkm_path), date_col="Date", value_col="Price").set_index("date").sort_index()
            ts["value"] = pd.to_numeric(ts["value"], errors="coerce")

            # Keep the *original trading-day series* to identify "real" points
            ts_orig = ts["value"].copy()

            # Build a daily index and forward-fill only (no backfill before first date)
            full_idx = pd.date_range(ts.index.min(), ts.index.max(), freq="D")
            ts_filled = ts_orig.reindex(full_idx).ffill()

            # Map both: original (real) and filled (includes weekends/holidays)
            mapped_real = df.index.to_series().map(ts_orig)
            mapped_full = df.index.to_series().map(ts_filled)

            # Apply values
            mask_full = mapped_full.notna()
            df.loc[mask_full, "jkm_usd_mmbtu"] = mapped_full[mask_full].values

            # Provenance:
            real_mask = mapped_real.notna()
            ffill_mask = (~real_mask) & mask_full
            prov.loc[real_mask,  "jkm_usd_mmbtu"] = f"csv:{jkm_path.name}"
            prov.loc[ffill_mask, "jkm_usd_mmbtu"] = f"csv-ffill:{jkm_path.name}"
        except Exception as e:
            print(f"[WARN] JKM CSV load failed from {jkm_path}: {e}")
